...
auto PreparedStatement::step() -> bool
{
    ...
    throw std::runtime_error(
        std::string("SQLite step error: ") + sqlite3_errmsg(sqlite3_db_handle(stmt_)));
}
...
TransactionGuard::TransactionGuard(sqlite3* db)
    : db_(db)
{
...
auto match_type_to_string(core::MatchType type) -> std::string_view
{
    switch (type)
    {
    case core::MatchType::Exact: return "exact";
    case core::MatchType::Sha1Only: return "sha1_only";
    case core::MatchType::Md5Only: return "md5_only";
    case core::MatchType::Crc32Only: return "crc32_only";
    case core::MatchType::SizeOnly: return "size_only";
    case core::MatchType::NoMatch: return "no_match";
    }
    return "unknown";
}
...
{
    if (str == "exact") return core::MatchType::Exact;
    if (str == "sha1_only") return core::MatchType::Sha1Only;
    if (str == "md5_only") return core::MatchType::Md5Only;
    if (str == "crc32_only") return core::MatchType::Crc32Only;
    if (str == "size_only") return core::MatchType::SizeOnly;
...
auto status_to_string(core::RomStatusType status) -> std::string_view
{
    switch (status)
    {
    case core::RomStatusType::Verified: return "verified";
    case core::RomStatusType::Missing: return "missing";
    case core::RomStatusType::Unverified: return "unverified";
    case core::RomStatusType::Mismatch: return "mismatch";
    }
    return "unknown";
}
...
{
    if (str == "verified") return core::RomStatusType::Verified;
    if (str == "missing") return core::RomStatusType::Missing;
    if (str == "unverified") return core::RomStatusType::Unverified;
    if (str == "mismatch") return core::RomStatusType::Mismatch;
...
auto Database::execute(std::string_view sql) -> Result<void>
{
    ...
    if (rc != SQLITE_OK)
    {
        ...
        return std::unexpected(
            core::Error{core::ErrorCode::DatabaseQueryError, "SQL error: " + err});
    }
    return {};
}
...
auto Database::prepare(std::string_view sql) -> Result<PreparedStatement>
{
    ...
    if (rc != SQLITE_OK)
    {
        return std::unexpected(core::Error{
            core::ErrorCode::DatabaseQueryError,
...
{
    auto stmt = prepare(
        "INSERT INTO dat_versions (system_id, name, version, source_url, checksum) "
        "VALUES (?1, ?2, ?3, ?4, ?5)");
    if (!stmt) return std::unexpected(stmt.error());
...
{
    auto stmt = prepare(
        "SELECT id, system_id, name, version, source_url, checksum, imported_at "
...
{
    auto stmt = prepare(
        "INSERT OR IGNORE INTO games (name, system_id, dat_version_id) "
        "VALUES (?1, ?2, ?3)");
    if (!stmt) return std::unexpected(stmt.error());
    ...
    if (id == 0)
    {
        ...
        auto find = prepare(
            "SELECT id FROM games WHERE name = ?1 AND system_id = ?2 AND dat_version_id = ?3");
...
{
    auto stmt = prepare(
        "SELECT id, name, system_id, dat_version_id FROM games WHERE dat_version_id = ?1");
    if (!stmt) return std::unexpected(stmt.error());
...
{
    auto stmt = prepare(
        "INSERT INTO roms (game_id, name, size, crc32, md5, sha1, region) "
...
auto Database::find_rom_by_sha1(std::string_view sha1)
    -> Result<std::optional<core::RomInfo>>
{
    auto stmt = prepare(
        "SELECT id, game_id, name, size, crc32, md5, sha1, region "
...
auto Database::find_rom_by_md5(std::string_view md5)
    -> Result<std::optional<core::RomInfo>>
{
    auto stmt = prepare(
        "SELECT id, game_id, name, size, crc32, md5, sha1, region "
...
auto Database::find_rom_by_crc32(std::string_view crc32)
...
{
    auto stmt = prepare(
        "SELECT r.id, r.game_id, r.name, r.size, r.crc32, r.md5, r.sha1, r.region "
...
auto Database::find_file_by_path(std::string_view path)
    -> Result<std::optional<core::FileInfo>>
{
    auto stmt = prepare(
        "SELECT id, path, size, crc32, md5, sha1, last_scanned FROM files WHERE path = ?1");
    if (!stmt) return std::unexpected(stmt.error());
    ...
    -> Result<std::int64_t>
{
    ...
    if (!all_files) return std::unexpected(all_files.error());
    ...
    if (!del_matches) return std::unexpected(del_matches.error());
...
{
    auto stmt = prepare(
        "INSERT OR REPLACE INTO file_matches (file_id, rom_id, match_type) "
        "VALUES (?1, ?2, ?3)");
    if (!stmt) return std::unexpected(stmt.error());
...
{
    auto stmt = prepare(
        "SELECT file_id, rom_id, match_type FROM file_matches WHERE file_id = ?1");
    if (!stmt) return std::unexpected(stmt.error());
...
auto Database::get_matches_for_rom(std::int64_t rom_id)
    -> Result<std::vector<core::MatchResult>>
{
    auto stmt = prepare(
        "SELECT file_id, rom_id, match_type FROM file_matches WHERE rom_id = ?1");
    if (!stmt) return std::unexpected(stmt.error());
...
auto Database::upsert_rom_status(std::int64_t rom_id, core::RomStatusType status)
    -> Result<void>
{
    auto stmt = prepare(
        "INSERT INTO rom_status (rom_id, status, last_updated) "
...
auto Database::get_rom_status(std::int64_t rom_id)
    -> Result<std::optional<core::RomStatusEntry>>
{
    auto stmt = prepare(
        "SELECT rom_id, status, last_updated FROM rom_status WHERE rom_id = ?1");
    if (!stmt) return std::unexpected(stmt.error());
...
{
    std::string sql =
        "SELECT g.name, r.name, s.name, r.sha1 "
        "FROM roms r "
        "JOIN games g ON r.game_id = g.id "
        "JOIN systems s ON g.system_id = s.id "
        "LEFT JOIN rom_status rs ON r.id = rs.rom_id "
