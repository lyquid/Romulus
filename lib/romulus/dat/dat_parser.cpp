...
auto DatParser::parse(const std::filesystem::path& dat_path)
    -> Result<core::DatFile>
{
    ...
    {
        return std::unexpected(core::Error{
            core::ErrorCode::DatInvalidFormat,
...
auto DatParser::parse_game(const void* node_ptr) -> Result<core::GameInfo>
{
    ...
    if (game.name.empty())
    {
        return std::unexpected(core::Error{
            core::ErrorCode::DatInvalidFormat,
            "Game entry missing 'name' attribute"});
    }

    // Parse all <rom> children
    for (auto rom_node = node.child("rom"); rom_node;
         rom_node = rom_node.next_sibling("rom"))
    {
        core::RomInfo rom;
