...
|-------|---------------|---------------|
| `dat_versions` | Each imported DAT file (name, version, SHA-256 checksum). Unique by checksum so the same file can't be imported twice. | The rulebook — what a given publisher decided correct ROMs look like. |
...
| `files` | Every ROM **file found on disk** (path, size, hashes). Points into `global_roms` via `sha1`. | *Reality* — what is actually sitting in your scan folders. |
